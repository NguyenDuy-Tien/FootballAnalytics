...
public class WhoScoreCrawler {
    ...
    public static List<PlayerSavingInfo> crawPlayerSaving() {
        List<PlayerSavingInfo> result = new ArrayList<>();
        //access and wait for web to load
        WebDriver driver = new ChromeDriver();
        driver.get("https://1xbet.whoscored.com/Statistics");
        new WebDriverWait(driver, Duration.ofSeconds(5)).until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("tbody[id='player-table-statistics-body']")));
        ...
        Select category = new Select(driver.findElement(By.cssSelector("select[id='category']")));
        category.selectByValue("saves");

        ((JavascriptExecutor) driver).executeScript("arguments[0].setAttribute('data-value','\\\'GK\\\'');", driver.findElement(By.cssSelector("div[id='positionOptions']")));


        ((JavascriptExecutor) driver).executeScript("arguments[0].click();", driver.findElement(By.cssSelector("div[id='top-player-stats-detailed']")).findElement(By.className("search-button")));
        waitTableLoad(driver);
        while (true) {
            WebElement statisticTable = driver.findElement(By.cssSelector("div[id='top-player-stats-detailed']")).findElement(By.cssSelector("tbody[id='player-table-statistics-body']"));
            ...
            for (WebElement element : elements) {
                try {
                    ...
                    String apps = element.findElements(By.cssSelector("td")).get(2).getText();
                    int minutePlayed;
                    float saveTotal, saveSixYardBox, savePenaltyArea, saveObox,rating;
                    ...
                    try {
                        saveTotal = Float.parseFloat(element.findElement(By.cssSelector("td[class='saveTotal   ']")).getText());
                    } catch (NumberFormatException e) {
                        saveTotal = 0;
                    }
                    try {
                        saveSixYardBox = Float.parseFloat(element.findElement(By.cssSelector("td[class='saveSixYardBox   ']")).getText());
                    } catch (NumberFormatException e) {
                        saveSixYardBox = 0;
                    }
                    try {
                        savePenaltyArea = Float.parseFloat(element.findElement(By.cssSelector("td[class='savePenaltyArea   ']")).getText());
                    } catch (NumberFormatException e) {
                        savePenaltyArea = 0;
                    }
                    try {
                        saveObox = Float.parseFloat(element.findElement(By.cssSelector("td[class='saveObox   ']")).getText());
                    } catch (NumberFormatException e) {
                        saveObox = 0;
                    }
                    try {
                    ...
                    System.out.println(name);
                    result.add(new PlayerSavingInfo(name, team,age,position, apps,minutePlayed,saveTotal,saveSixYardBox,savePenaltyArea,saveObox,rating));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
